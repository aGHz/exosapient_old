...
def syntax():
    print """Generate instructions to deploy this new installation of exosapient
...
def nginx(path, linux):
    ...
    out += [
        ...
        "# " + '-' * 72,
        "{0}ln -s /Users/tek/src/exosapient/etc/nginx.conf {1}".format('sudo ' if linux else '', os.path.join(path, 'exosapient')),
        "",
        ]
...
def auto(user_group, linux):
    ...
    if linux:
        out += [
            ...
            "sudo ln -s /Users/tek/src/exosapient/bin/initd.sh /etc/init.d/exosapient",
            "sudo update-rc.d exosapient defaults",
            "",
            "echo",
            "echo " + '-' * 80,
            "echo '    To no longer start on boot, run:'",
            "echo '        sudo /etc/init.d/exosapient stop'",
            "echo '        sudo update-rc.d -f exosapient remove'",
            "echo " + '-' * 80,
            "echo",
            "",
            ]
        ...
        out += [
            ...
            "ln -s /Users/tek/src/exosapient/bin/launchAgent.plist ~/Library/LaunchAgents/com.exosapient.tek.production.plist",
            "launchctl load ~/Library/LaunchAgents/com.exosapient.tek.production.plist",
            "echo",
            "echo " + '-' * 80,
            "echo '    To no longer start on boot, run:'",
            "echo '        launchctl stop com.exosapient.tek.production'",
            "echo '        launchctl remove com.exosapient.tek.production'",
            "echo '        rm ~/Library/LaunchAgents/com.exosapient.tek.production.plist'",
            "echo " + '-' * 80,
            "echo",
            "",
            ]
...
def start(opt, linux):
    ...
    if '--auto' in opt:
        ...
        if linux:
            out += [
                "echo '        sudo /etc/init.d/exosapient start'",
                "sudo /etc/init.d/exosapient start",
                ]
        else:
            out += [
                "echo '        launchctl start com.exosapient.tek.production'",
                "launchctl start com.exosapient.tek.production",
                ]
        out += [
            "echo " + '-' * 80,
            "",
    ...
    if '--auto' in opt:
        ...
        if linux:
            out += ["echo '    sudo /etc/init.d/exosapient start|stop|restart'"]
        else:
            out += ["echo '    launchctl start|stop com.exosapient.tek.production'"]
    out += [
        "echo " + '-' * 80,
        "echo",
        "",
